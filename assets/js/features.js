...
function toggleCompare(key) {
  ...
  document
    .querySelectorAll(`[data-act="cmp"][data-key="${CSS.escape(k)}"]`)
    .forEach((btn) => {
  ...
  if (panelCmpBtn) {
    ...
    try {
      panelCmpBtn.setAttribute("aria-pressed", inCmp ? "true" : "false");
    } catch (_e) {}
  }
  // Update compare modal if open - only update button states, don't reopen
  if (compareModal && compareModal.classList.contains("on")) {
...
tbody.addEventListener("mouseout", (e) => {
  if (!e.target.closest("tr[data-key]")) hideRowTip();
});
...
cgrid.addEventListener("mouseout", (e) => {
  if (!e.target.closest(".pcard[data-key]")) hideRowTip();
});
