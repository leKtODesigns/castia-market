...
function buildCatFilter() {
  ...
  const cats = [...new Set(enriched.map((r) => r.category))].sort(
    (a, b) =>
      enriched.filter((r) => r.category === b).length -
      enriched.filter((r) => r.category === a).length,
  );
  catEl.innerHTML = '<option value="">All categories</option>';
  cats.forEach((c) => {
    const o = document.createElement("option");
    o.value = c;
    o.textContent = `${CAT_LABELS[c] || c} (${enriched.filter((r) => r.category === c).length})`;
    catEl.appendChild(o);
  });
  catEl.value = want;
  if (catEl.dataset) delete catEl.dataset.restore;
  $("sCat").textContent = cats.length;
  if (typeof refreshCSelect === "function") {
...
function render() {
  ...
  const hasF =
    qEl.value.trim() || catEl.value || confEl.value || tierEl.value !== "";
  $("ri").textContent = hasF
...
function renderPag(pages) {
  // Update the page‑info line (both locations)
  $("pmeta").textContent = filtered.length
  ...
  if (elTop) elTop.innerHTML = b.join("");
  ...
  if (pages <= 1) {
    ...
    elBottom.innerHTML = elTop ? elTop.innerHTML : "";
  }
}
...
function updateSortUI() {
  ...
  // Update the text of the last_seen button to show Newest / Oldest
  const tsBtn = $("sp-lastseen");
  if (tsBtn) {
    tsBtn.textContent = sd === "asc" ? "Oldest" : "Newest";
  }
}
